...
class AuthManager {
    constructor() {
        ...
        this.isProcessingLogin = false; // Guard against multiple login events
        this.initializeAuth();
    }
    ...
    async handleLoginSuccess() {
        ...
        try {
            ...
            // Navigate to master dataset (main page) first
            if (window.navigation) {
                window.navigation.navigateTo('master-dataset');
            }
            
            // Fire auth:login event for modules to listen to (only once)
            console.log('Dispatching auth:login event');
            ...
            // Reset the guard after a shorter delay to allow retries
            setTimeout(() => {
    ...
    async notifyAuthenticationComplete() {
        // This method is no longer used to prevent infinite loops
        // Auth events are now handled via the 'auth:login' event only
        return;
        
        // Small delay to ensure backend has completed user profile creation
        ...
        // Reload dataset module with additional delay for new users
        if (window.dataset) {
            try {
        ...
        // Reload job analysis history
        if (window.jobAnalysis) {
            try {
                await window.jobAnalysis.loadAnalysisHistory();
            } catch (error) {
                console.error('Failed to load job analysis history:', error);
            }
        }
        
        // Reload content selection
        if (window.contentSelection) {
            try {
                await window.contentSelection.loadMasterDataset();
            } catch (error) {
                console.error('Failed to load content selection data:', error);
            }
        }
        
        // Check LaTeX status for resume preview
        if (window.resumePreview) {
            try {
                await window.resumePreview.checkLatexStatus();
            } catch (error) {
