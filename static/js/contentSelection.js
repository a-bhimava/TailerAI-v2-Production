...
class ContentSelectionManager {
    ...
    initialize() {
        ...
        window.addEventListener('auth:login', () => {
            // Load data after successful login if we're on content selection view
            const currentRoute = window.navigation?.getCurrentRoute();
            if (currentRoute === 'content-selection') {
                this.loadJobAnalysis();
                this.loadMasterDataset();
            }
        });
        
        // Check if we should load data now (if already authenticated and on the right page)
    ...
    async loadMasterDataset() {
        ...
        if (!window.api.isAuthenticated()) {
            console.log('ContentSelection: User not authenticated, skipping dataset load');
