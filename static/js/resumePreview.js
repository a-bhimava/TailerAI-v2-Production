...
class ResumePreviewManager {
    ...
    initialize() {
        ...
        window.addEventListener('auth:login', () => {
            // Load data after successful login if we're on resume preview view
            const currentRoute = window.navigation?.getCurrentRoute();
            if (currentRoute === 'resume-preview') {
        ...
        window.addEventListener('navigation:routeChanged', (event) => {
            if (event.detail.route === 'resume-preview' && window.api.isAuthenticated()) {
                this.loadSelectionData();
                this.checkLatexStatus();
            }
        });
