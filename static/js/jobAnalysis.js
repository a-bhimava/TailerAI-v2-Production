...
class JobAnalysisManager {
    ...
    initialize() {
        ...
        window.addEventListener('auth:login', () => {
            // Load history after successful login if we're on job analysis view
            const currentRoute = window.navigation?.getCurrentRoute();
            if (currentRoute === 'job-analysis') {
        ...
        window.addEventListener('navigation:routeChanged', (event) => {
            if (event.detail.route === 'job-analysis' && window.api.isAuthenticated()) {
                this.loadAnalysisHistory();
            }
        });
        
        // Check if we should load history now (if already authenticated and on the right page)
