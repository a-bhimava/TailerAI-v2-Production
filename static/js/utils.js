...
const Performance = {
    ...
    time(label, func) {
        return async (...args) => {
            ...
            // Performance timing available for debugging if needed
            return result;
        };
    },
 ...
 */
async function downloadFileWithAuth(url) {
    try {
        const token = localStorage.getItem('tailerai_token');
        if (!token) {
        ...
        window.notifications.show('Failed to download file. Please try again.', 'error');
    }
}